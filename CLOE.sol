...
contract CLOE_ERC223 is Ownable, IERC223 {
  ...
  string public constant symbol = "CLOE";
  string public constant name = "Callisto Enterprise Token";
  event SetMinter(address previousMinter, address newMinter);
  event Rescued(address token, uint256 value);
  

  /**
   * @dev Migrate from old version of token to this one
   * @param amount - amount of token to migrate 
   */
  function migrate(uint256 amount) external {
    CLOE_ERC223 oldCLOE = CLOE_ERC223(0x1eAa43544dAa399b87EEcFcC6Fa579D5ea4A6187);
    oldCLOE.burnFrom(msg.sender, amount);
  ...
  */
  ...
  function rescueERC20(address token) external onlyOwner {
    ...
    (bool success,) = token.call(abi.encodeWithSelector(0xa9059cbb, msg.sender, value));
    require(success, "call failed");
    emit Rescued(token, value);
  }
